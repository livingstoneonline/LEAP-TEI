...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="p[child::pb]" priority="10">
		<p class="{concat(name(), ' ', translate(@rend, '-', ''), ' ', 'fake-align')}">
			<xsl:apply-templates/>
		</p>
	</xsl:template>
	...
	<xsl:template match="fw[@type='pageno'][parent::p]">
		<span class="{concat(name(), ' ', @type, ' ', @rend)}" title="">
			<xsl:apply-templates/><br/>
		</span>
		<xsl:text disable-output-escaping="yes">&lt;p rend="no-indent" class="fake-p"&gt;&lt;span&gt;</xsl:text>
	</xsl:template>
...
<xsl:template match="idno[@type='LEAP-ID']">
	...
	<xsl:template match="pb[parent::p]" priority="10"><!-- Add for LJ -->
